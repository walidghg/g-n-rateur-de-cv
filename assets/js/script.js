document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('input-photo').addEventListener('change', function(e) {
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                ...
                container.innerHTML = `<img src="${e.target.result}" style="width:120px;height:120px;border-radius:50%;object-fit:cover;border:4px solid #2c3e50;margin-bottom:15px;box-shadow: 0 5px 15px rgba(0,0,0,0.1);">`;
            }
    ...
    document.getElementById('btn-add-exp').onclick = () => {
        ...
        div.className = 'experience-item card p-3 mb-3';
        div.innerHTML = `
            <div class="d-flex justify-content-between mb-2">
                <input type="text" name="experiences[${counts.exp}][poste]" class="form-control input-exp-poste" placeholder="Poste">
                <button type="button" class="btn btn-sm btn-outline-danger btn-remove rounded-circle" style="width:30px;height:30px;padding:0;">X</button>
            </div>
    ...
    document.getElementById('btn-add-formation').onclick = () => {
        const div = document.createElement('div');
        div.className = 'formation-item card p-3 mb-3';
        div.innerHTML = `
            <div class="d-flex justify-content-between mb-2">
    ...
    document.getElementById('btn-add-skill').onclick = () => {
        const div = document.createElement('div');
        div.className = 'competence-item row mb-2 align-items-center';
        div.innerHTML = `
            ...
            <div class="col-1"><button type="button" class="btn btn-outline-danger btn-sm rounded-circle btn-remove" style="width:30px;height:30px;padding:0;">X</button></div>
        `;
